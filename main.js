...
// Custom Photo Video Integration 
...
function clearUploadedMedia() {
  if (uploadedMediaType === "video" && uploadedMedia) {
    uploadedMedia.pause();
    uploadedMedia.removeAttribute("src");
    uploadedMedia.load();
  }
...
mediaInput.addEventListener("change", async (event) => {
  ...
  if (file.type.startsWith("image/")) {
    ...
    mediaVideo.addEventListener("loadeddata", async () => {
    ...
    mediaVideo.src = uploadedMediaURL;
    mediaVideo.load();
...
function drawUploadedMedia(q) {
  if (!uploadedMedia || !uploadedMediaType || !q) return;
  ...
  // The finger frame is ONLY a mask.
  // The media itself occupies the entire canvas.
  quadPath(ctx, q);
  ctx.clip();
